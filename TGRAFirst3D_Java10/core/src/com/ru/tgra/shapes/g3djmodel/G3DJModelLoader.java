...
public class G3DJModelLoader {
	...
	public static MeshModel loadG3DJFromFile(String fileName)
	{
		...
		for(Object meshDesc : meshes)
		{
			...
			for(Object partDesc : parts)
			{
				...
				//System.out.println("capacity: " + part.indices.capacity());
		...
		JsonObject j = ((JsonObject)graph.get("nodes"));
		if (j != null) {
			System.out.println("w00t");
			j = (JsonObject)j.getArray()[0];
			System.out.println(j.containsKey("children"));
		}
		
		if (j.containsKey("children")) {
			nodes = getObjectArray(j, "children");
		}else {
			nodes = getObjectArray(graph, "nodes");
		}
	...
	private static Object[] getObjectArray(Object jsonObject, String key, String key2)
	{
		if(((JsonObject)jsonObject).get(key) == null) return null;
		if(((JsonObject)((JsonObject)jsonObject).get(key)).get(key2) == null) return null;
		JsonObject j = (JsonObject) ((JsonObject)((JsonObject)jsonObject).get(key)).get(key2);
		
		return (Object[])(j.getArray());
	}
