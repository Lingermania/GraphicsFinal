...
public class Camera {
	...
	public Vector3D direction(Point3D point) {
		//Returns directional vector to point
		
		Vector3D v = Vector3D.difference(eye, point);
		v.normalize();
		return v;
	}
	...
	public void walkForward(float del) {
		eye.x -= del*n.x;
		//eye.y -= del*n.y; //Remove this
		eye.z -= del*n.z;
	}
	public void walkForwardX(float del) {
		eye.x -= del*n.x;
	}
	public void walkForwardZ(float del) {
		eye.z -= del*n.z;
	}
